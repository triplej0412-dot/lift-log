...
The app uses the same `users/{uid}/workouts` Firestore collection and ships the exact 363-preset friend catalog. Its set model preserves raw kg canonical values and the V2 `inputLoadValue` / `inputLoadUnit` fields. The analysis server URL defaults to the production Render HTTPS endpoint configured as `ANALYSIS_BASE_URL` in `app/build.gradle.kts`.
